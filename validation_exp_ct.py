...
def main(_run, _config, _log):
    ...
    # temp_path = '/date/wjp/Fed_fewshot_proto/temp_CT/FedOurs-S1/V{}/'.format(_config['eval_fold'])
    temp_path = '/home/b311/data1/wjp/Fed_fewshot_proto/temp_CT/FedOurs-S1-l4/V{}/'.format(_config['eval_fold'])
    # temp_path = '/home/b311/data1/wjp/Fed_fewshot_proto/temp_CT/FedOurs-oth/our-s2-2/V{}/'.format(_config['eval_fold'])

    # temp_txt_path = '/date/wjp/Fed_fewshot_proto/test_info/'
    temp_txt_path = '/home/b311/data1/wjp/Fed_fewshot_proto/test_info/'
    # txt_path = temp_txt_path + 'test_reslult_4utrain(FedProto-S1-us-4k-1)_ct.txt'
    # txt_path = temp_txt_path + 'test_reslult_4utrain(FedOurs-S2-4k-l4as-3)_ct.txt'
    txt_path = temp_txt_path + 'test_reslult_4utrain(FedOurs-S1-us-4k-self-1)_ct.txt'
    vfold_result = []
    mean_Dice_Sp = []
    mean_Dice_RK = []
    mean_Dice_LK = []
    ...
    for sr in range(0,7,1):
        ...
        for name in files_name:
            ...
            # para_path = temp_path + '{}_round:{}.pth'.format(client_idx, sr)  # FedProto
            para_path = temp_path + '{}_{}.pth'.format(client_idx, sr)  # FedOurs

            _log.info(f'###### Reload model {_config["reload_model_path"]} ######')
            model = FewShotSeg(pretrained_path=para_path, cfg=_config['model'])
            model = model.cuda()
            model.eval()
            ...
            with open(txt_path, 'a', encoding='utf-8') as f:
                print("------------------------------------------------------------------", file=f)
                print("dataset:", baseset_name, "com_round:", sr, para_path,file=f)
